...
var NumArray = function(nums) {
    ...
    this.nums.forEach((v, i) => {
        this.updateTree(i + 1, v);
    });
};

NumArray.prototype.updateTree = function(i, v) {
    while (i <= this.nums.length) {
        this.bit[i] += v;
        i += (i & -i); //add lowbit, 3 -> 4 -> 8, -i = ~i + 1, i & -i = lowest bit of 1
    }
}
...
NumArray.prototype.query = function(i) {
    ...
    while (i > 0) {
        sum += this.bit[i];
        i -= (i & -i); //query - lowbit, 7 -> 6 (5, 6)-> 4 (4, 3, 2, 1)
    }
    return sum;
};

NumArray.prototype.sumRange = function(i, j) {
    return this.query(j + 1) - this.query(i); //i, j included
};
