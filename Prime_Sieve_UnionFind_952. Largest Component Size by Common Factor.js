...
const sieve = N => {
    ...
    let p = 2;
    while (p <= Math.sqrt(N)) {//the max of the smaller factor of two factors, N = f1 * f2, if f1 is smaller, f1 can not exceed sqrt
        if (primes[p]) {
...
var largestComponentSize = function(A) {
    const primes = sieve(~~(Math.max(...A)/2));//the largest prime factor can not be great than N/2
    ...
    const rank = Array(primes.length + A.length).fill(0);
    const find = v => {
        if (parent[v] !== v) {
