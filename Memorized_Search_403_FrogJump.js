...
var canCross = function(stones) {
    ...
    const dp = [...Array(stones.length)].map(() => Array(stones.length));
    const map = stones.reduce((a, c, i) => {a[c] = i; return a;}, {});
    const dfs = (i, j) => {
        if (i > stones.length || dp[i][j] !== undefined) return;
        dp[i][j] = true;
        for (let k = -1; k <= 1; k++) {
            if (map[stones[i] + j + k]) dfs(map[stones[i] + j + k], j + k);
        }
    };
    dfs(1, 1);
    return dp[stones.length - 1].some(c => c);
};
